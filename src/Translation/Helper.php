...
class Helper
{
     ...
     * Get plural form.
     */
    public static function getPluralForm(string $langcode): ?string
    {
        return self::PLURAL_TABLE[$langcode][1] ?? null;
    }

    public static function getNumberOfPlurals(string $langcode): int
    {
        $pluralForm = self::getPluralForm($langcode);
        preg_match('/nplurals=(\d)+;/', (string) $pluralForm, $matches);
